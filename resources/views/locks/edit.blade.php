...
@section('title', 'Editar Cerradura')

@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="bg-white rounded-lg shadow-md p-6">
            <h2 class="text-2xl font-bold mb-6">Editar Cerradura</h2>

            <form action="{{ route('locks.update', $lock) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <div class="space-y-2">
                        <label class="inline-flex items-center">
                            <input type="radio" name="location_type" value="apartment" class="form-radio" {{ $lock->apartment_id ? 'checked' : '' }} onclick="toggleLocation('apartment')">
                            <span class="ml-2">Apartamento</span>
                        </label>
                        <label class="inline-flex items-center ml-6">
                            <input type="radio" name="location_type" value="building" class="form-radio" {{ $lock->building_id ? 'checked' : '' }} onclick="toggleLocation('building')">
                            <span class="ml-2">Puerta Principal (Edificio)</span>
                        </label>
                    </div>
                </div>

                <div id="apartment_select" class="mb-4 {{ $lock->building_id ? 'hidden' : '' }}">
                    ...
                    <select name="apartment_id" id="apartment_id"
                        ...
                        @foreach($apartments as $apartment)
                            <option value="{{ $apartment->id }}" {{ $lock->apartment_id == $apartment->id ? 'selected' : '' }}>
                ...
                <div id="building_select" class="mb-4 {{ $lock->apartment_id ? 'hidden' : '' }}">
                    ...
                    <select name="building_id" id="building_id"
                        ...
                        @foreach(\App\Models\Building::where('active', true)->get() as $building)
                            <option value="{{ $building->id }}" {{ $lock->building_id == $building->id ? 'selected' : '' }}>
                                {{ $building->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="name" id="name" value="{{ $lock->name }}"
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="device_id" id="device_id" value="{{ $lock->device_id }}"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                        required>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" name="model" id="model" value="{{ $lock->model }}"
                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>

                <div class="mb-6">
                    <label class="flex items-center">
                        <input type="checkbox" name="active" value="1" {{ $lock->active ? 'checked' : '' }}
